...
struct AhoCorasick {
    ...
    void insert(string s) {
        int u = 0;
        for(char c : s) {
            if(!trie[u].next[c-'a']) { trie[u].next[c-'a'] = trie.size(); trie.emplace_back(); }
            u = trie[u].next[c-'a'];
        }
        trie[u].end++;
    }
    void build() {
        queue<int> q;
        for(int i = 0; i < 26; i++) if(trie[0].next[i]) q.push(trie[0].next[i]);
        while(!q.empty()) {
            int u = q.front(); q.pop();
            for(int i = 0; i < 26; i++) {
...
int main() {
    AhoCorasick ac; ac.insert("he"); ac.insert("she"); ac.build();
    return 0;
}
